...
def format_for_process(params):
    ...
    for k, v in params.items():
        k = k.rjust(18)
        v = '{:<12}'.format(round(v,6))[:12]
        new_params.append([k, v])
    return new_params

'''
Based on mbpo's Progress.

https://github.com/JannerM/mbpo
'''
class Progress:

	def __init__(self, total, name = 'Progress', ncol=3, max_length=30, indent=0, line_width=100, speed_update_freq=50):
		self.total = total
		self.name = name
		self.ncol = ncol
